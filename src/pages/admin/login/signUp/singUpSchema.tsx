import { z } from 'zod';

export const singUpSchema = z.object({
  email: z
    .string({ required_error: 'Поле повинно бути заповнене' })
    .refine(
      (value) =>
        /^[a-zA-Z0-9._%+-]+@(?!.*\.(ru|by))[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/.test(
          value
        ),
      {
        message: 'Введіть коректний email'
      }
    ),
  password: z
    .string({ required_error: 'Поле повинно бути заповнене' })
    .min(8, 'Пароль має містити мінімум 8 символів')
    .max(14, 'Пароль має містити максимум 14 символів')
    .refine(
      (value) =>
        /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!-_)(.,])/.test(value),
      {
        message:
          'Пароль має містити хоча б одну велику літеру, один спеціальний символ, одну цифру та малі літери'
      }
    ),
  confirmpassword: z.string({ required_error: 'Поле повинно бути заповнене' })
});

singUpSchema.refine((data) => data.confirmpassword === data.password, {
  message: 'Паролі не збігаються. Спробуйте ще',
  path: ['confirmPassword']
});
